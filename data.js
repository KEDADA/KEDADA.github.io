...
window.PORTFOLIO_DATA = {
  ...
  timeline: [
    ...
    },
    {
      ...
      contributions: ["以 Dispatch Table 动态注册、Batch Tool Calling 和 Interrupt 中断恢复扩展 Agent Loop；新增工具不侵入核心循环。", "构建 Episodic JSONL + Semantic Vector 双层记忆，以及 Context 四操作主动压缩策略。", "以 60 Case 隔离测试集门控技能进化；6 轮迭代后任务成功率从 52% 提升至 80%。", "实现三级权限模型、Dry-run 副作用预览与成本守卫；预设高危操作拦截率 100%。"],
      approach: ["Dispatch Table 将工具发现、Schema 与执行器从 Agent Loop 解耦。", "所有技能提案先进入隔离沙箱，通过固定回归集且得到用户确认后才集成。", "权限、Dry-run 与成本守卫位于统一执行边界。"]
    },
    {
      ...
      approach: ["将工具注册、Schema 校验、权限判断和错误结构收敛到统一调用边界。", "以分层作用域约束记忆读写，支持持久化、恢复和跨作用域泄漏防护。", "检索结果保留来源、评分和选择理由，并用固定查询集持续回归。"],
      link: { label: "在 GitHub 查看 learn-workbuddy", url: "https://github.com/KEDADA/learn-workbuddy" }
    },
    {
